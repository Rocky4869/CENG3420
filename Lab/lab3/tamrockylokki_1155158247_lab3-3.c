...
void eval_bus_drivers() {
    ...
    /* input of GateMAR */
    // value_of_GateMAR = ?;
    value_of_GateMAR = mar_mux(get_MARMUX(CURRENT_LATCHES.MICROINSTRUCTION), value_of_MARMUX,
                               mask_val(CURRENT_LATCHES.IR, 31, 12) << 20);
    printf("Lab3-3 assignment: value_of_GateMAR = ?;\n");
    ...
    value_of_shift_function_unit = rs2_mux(get_RS2MUX(CURRENT_LATCHES.MICROINSTRUCTION),
                                               rs1_en(
                                                       get_RS2En(CURRENT_LATCHES.MICROINSTRUCTION),
                                                       0,
                                                       CURRENT_LATCHES.REGS[mask_val(CURRENT_LATCHES.IR, 24, 20)]),
    ...
    printf("Lab3-3 assignment: value_of_shift_function_unit = ?;\n");
    ...
    value_of_GateALUSHF = alu_shift_mux(mask_val(CURRENT_LATCHES.IR, 14, 12),
                                        alu(mask_val(CURRENT_LATCHES.IR,14,12),
                                            mask_val(CURRENT_LATCHES.IR, 31, 25),
                                        rs1_en(get_RS1En(CURRENT_LATCHES.MICROINSTRUCTION),
                                               0,
                                               CURRENT_LATCHES.REGS[mask_val(CURRENT_LATCHES.IR, 19, 15)]),
                                            rs2_mux(
                                                    get_RS2MUX(CURRENT_LATCHES.MICROINSTRUCTION),
            ...
            shift_function_unit(mask_val(CURRENT_LATCHES.IR, 14, 12),
                                mask_val(CURRENT_LATCHES.IR, 31, 25),
                                rs1_en(get_RS1En(CURRENT_LATCHES.MICROINSTRUCTION),
                                       0,
                                       CURRENT_LATCHES.REGS[mask_val(CURRENT_LATCHES.IR, 19, 15)]),
    ...
    printf("Lab3-3 assignment: value_of_GateALUSHF = ?;\n");
     ...
     */
    /* input of GateRS2 */
    value_of_GateRS2 = rs2_en(get_RS2En(CURRENT_LATCHES.MICROINSTRUCTION),
                              0,
                              CURRENT_LATCHES.REGS[mask_val(CURRENT_LATCHES.IR, 24, 20)]
    );
    printf("Lab3-3 assignment: value_of_GateRS2 = ?;\n");
...
void drive_bus() {
    ...
    switch ((_GateMDR << 4) + (_GateRS2 << 3) + (_GatePC << 2) + (_GateALUSHF << 1) + (_GateMAR)) {
        ...
        case 1:
            ...
            printf("Lab3-3 assignment: when value = 1, BUS = ?;\n");
        case 2:
            /*
             *  Lab3-3 assignment
             */
            BUS = value_of_GateALUSHF;
            printf("Lab3-3 assignment: when value = 2, BUS = ?;\n");
        case 4:
            /*
             *  Lab3-3 assignment
             */
            BUS = value_of_GatePC;
            printf("Lab3-3 assignment: when value = 4, BUS = ?;\n");
        case 8:
            /*
             *  Lab3-3 assignment
             */
            BUS = valueOfGateRS2;
            printf("Lab3-3 assignment: when value = 8, BUS = ?;\n");
        case 16:
            /*
             *  Lab3-3 assignment
             */
            BUS = value_of_GateMDR;
            printf("Lab3-3 assignment: when value = 16, BUS = ?;\n");
        default:
            BUS = 0;
